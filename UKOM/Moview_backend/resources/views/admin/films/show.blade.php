        ...
        <div class="bg-white rounded-lg shadow p-6" x-data="{ activeSection: 'media', activeTab: 'posters', currentTab: 'posters' }">
            ...
            <div x-show="activeSection === 'metadata'" x-transition>
                <div class="space-y-8">
                    ...
                    <!-- Related Films (read-only preview, ordered by sort_order) -->
                    <div>
                        <label class="block text-lg font-semibold text-gray-800 mb-4">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            @if(isset($relatedMovies) && $relatedMovies->count() > 0)
                                ...
                                <p class="mt-3 text-xs text-gray-500"><i class="fas fa-info-circle mr-1"></i>Urutan sesuai sort_order (drag di form). Simetris — film ini muncul di related film tujuan.</p>
                            @else
                                <p class="text-gray-500 text-sm">Belum ada related film. Tambahkan di <a href="{{ route('admin.films.edit', $movie->id) }}" class="text-blue-600 hover:underline">Edit Film → Related Films</a>.</p>
                            @endif
                        </div>
                    </div>

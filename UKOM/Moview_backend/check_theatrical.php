...
foreach (DB::select("SELECT column_name AS \"Field\", data_type AS \"Type\" FROM information_schema.columns WHERE table_name = 'services' ORDER BY ordinal_position") as $c) echo $c->Field . '(' . $c->Type . ') ';
echo "\n\n=== movie_services columns ===\n";
foreach (DB::select("SELECT column_name AS \"Field\", data_type AS \"Type\" FROM information_schema.columns WHERE table_name = 'movie_services' ORDER BY ordinal_position") as $c) echo $c->Field . '(' . $c->Type . ') ';
echo "\n\n=== service types ===\n";

...
export const getHomeFeed = async ({ viewerId = null, cursor = null, limit = 20, mode = 'for-you', seed = '', recentInterestTags = [] }) => {
  if (mode === 'recent') {
  ...
  if (cursor?.score !== undefined && cursor?.score !== null) {
    ...
    cursorClause = `and (scored.score, scored."publishedAt", scored.id) < ($5::float, $6::timestamptz, $7::uuid)`
  } else if (cursor) {
    values.push(cursor.publishedAt, cursor.id)
    cursorClause = `and (scored."publishedAt", scored.id) < ($5::timestamptz, $6::uuid)`
  }
  ...
  const { rows } = await query(
    `${postSelect}
     where lower(u.username) = lower($2)
       and (
         (p.status = 'published' and (p.visibility = 'public' or p.author_id = $1))
         or (p.status = 'draft' and p.author_id = $1)
       )
       ${cursorClause}
     order by p.published_at desc, p.id desc
...
export const searchPosts = async ({ viewerId = null, q = '', tags = [], sort = 'relevance', limit = 30, offset = 0 }) => {
  ...
  if (searchTerm) {
    ...
    where.push(`(
      p.search_vector @@ plainto_tsquery('simple', $${qIndex})
    ...
    `with tag_matches as (
       ...
       where p.status = 'published'
         ...
         and (
           ...
           or regexp_replace(lower(tag_value.tag), '[^a-z0-9]+', '', 'g') like '%' || regexp_replace($1, '[^a-z0-9]+', '', 'g') || '%'
         )
       limit $2
     ),
     title_matches as (

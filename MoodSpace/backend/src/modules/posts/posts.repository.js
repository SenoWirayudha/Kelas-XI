    ...
    `with current_post as (
     ...
     current_tags as (
        select tag, tag_key from (
          ...
          select
            lower(ai_tag.tag),
            regexp_replace(lower(ai_tag.tag), '[^a-z0-9]+', '', 'g')
          from current_post cp
          cross join lateral jsonb_array_elements_text(coalesce(cp.metadata->'autoGeneratedTags', '[]'::jsonb)) ai_tag(tag)
        ) all_current
      ),
     candidate_scores as (
       select base_posts.*,
         (
            0.40 * least(1, coalesce((
              select count(*)::float
              from (
                select lower(t.tag) as tag, regexp_replace(lower(t.tag), '[^a-z0-9]+', '', 'g') as tag_key
                from jsonb_array_elements_text(coalesce(base_posts.metadata->'tags', '[]'::jsonb)) t(tag)
                union
                select lower(t.tag), regexp_replace(lower(t.tag), '[^a-z0-9]+', '', 'g')
                from jsonb_array_elements_text(coalesce(base_posts.metadata->'autoGeneratedTags', '[]'::jsonb)) t(tag)
              ) candidate_all
              join current_tags ct
                on ct.tag = candidate_all.tag
                or ct.tag_key = candidate_all.tag_key
  ...
  if (searchTerm) {
    ...
    where.push(`(
      ...
      or exists (
        ...
        where pm.post_id = p.id
          and (
            lower(coalesce(ua.title, '')) like '%' || lower($${qIndex}) || '%'
            or lower(coalesce(ua.description, '')) like '%' || lower($${qIndex}) || '%'
          )
      )
      or exists (
        select 1
        from jsonb_array_elements_text(coalesce(p.metadata->'autoGeneratedTags', '[]'::jsonb)) ai_search(tag)
        where lower(ai_search.tag) like '%' || lower($${qIndex}) || '%'
      )
    )`)
  }
    ...
    : sort === 'popular'
      ...
      : ftsIndex
        ? `(
             ...
             + case when exists (
                 select 1
                 from jsonb_array_elements_text(coalesce(p.metadata->'autoGeneratedTags', '[]'::jsonb)) ai_rank(tag)
                 where lower(ai_rank.tag) like '%' || lower($${qIndex}) || '%'
               ) then 2 else 0 end
           ) desc,
           p.like_count desc,
           p.save_count desc,
...
export const updatePostEmbedding = async ({ postId, embedding, textEmbedding = null, autoGeneratedTags = [], matchedEntity = null, ocrText = null }) => {
  const tagsJson = JSON.stringify(autoGeneratedTags)
  const entityJson = matchedEntity ? JSON.stringify(matchedEntity) : 'null'
  const ocrJson = ocrText ? JSON.stringify(ocrText) : 'null'
  const { rows } = await query(
    `update posts
     set embedding = $2::jsonb,
         text_embedding = $5::jsonb,
         metadata = jsonb_set(
           jsonb_set(
             jsonb_set(
     ...
     where id = $1
     returning id`,
    [postId, JSON.stringify(embedding), tagsJson, entityJson, JSON.stringify(textEmbedding), ocrJson],
  )
  return rows[0] || null
}
...
export const getPostsWithTextEmbedding = async ({ viewerId = null, limit = 500 }) => {
  const { rows } = await query(
    `${postSelect}
     where p.status = 'published'
       and (p.visibility = 'public' ${followerVisibilitySql})

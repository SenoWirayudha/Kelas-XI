...
export const recommendedPosts = async ({ viewerId = null, postId, query: params }) => {
  ...
  const semanticText = enrichForClipRerank(semanticParts.filter(Boolean).join(' ').trim())
  ...
  // Limit per keyword word: prevents any single word from dominating visItems
  const WORD_LIMIT = 2
  const wordCounts = {}
  const deduped = []
  for (const item of visItems) {
    const candidateText = `${item.title || ''} ${item.caption || ''} ${[...(item.metadata?.tags || []), ...(item.metadata?.autoGeneratedTags || [])].join(' ')}`.toLowerCase()
    const matchedWords = textFilterWords.filter(w => candidateText.includes(w))
    const canAdd = matchedWords.length === 0 || matchedWords.some(w => (wordCounts[w] || 0) < WORD_LIMIT)
    if (canAdd) {
      matchedWords.forEach(w => { wordCounts[w] = (wordCounts[w] || 0) + 1 })
      deduped.push(item)
    }
  }
  visItems = deduped

  ...
  try {
    ...
    if (combined) {
      ...
      if (visEntity && ocrEntity) {
        matchedEntity = visEntity.score >= ocrEntity.score ? visEntity : ocrEntity
        console.log('[EMBED] Dual-path entity match:', { visual: visEntity.title, visScore: visEntity.score.toFixed(3), ocr: ocrEntity.title, ocrScore: ocrEntity.score.toFixed(3), winner: matchedEntity.title })
      } else {
...
export const recommendedPosts = async ({ viewerId = null, postId, query: params }) => {
  ...
  // Semantic candidates: CLIP text-to-image threshold only (broader match)
  const txtItems = textEmb
    ? scored.filter(p => p._txtScore >= CLIP_SCORE_THRESHOLD).sort((a, b) => b._txtScore - a._txtScore)
    : []

  // Interleave: [vis1, txt1, vis2, txt2, ...]
  const interleaved = []
  const seenIds = new Set()
  const maxLen = Math.max(visItems.length, txtItems.length)
  for (let i = 0; i < maxLen && interleaved.length < params.limit; i++) {
    for (const list of [visItems, txtItems]) {
      if (i < list.length && !seenIds.has(list[i].id)) {
        seenIds.add(list[i].id)
        interleaved.push(list[i])
        if (interleaved.length >= params.limit) break
      }
    }
  }

  const hasMore = interleaved.length > params.limit
  const trimmed = hasMore ? interleaved.slice(0, params.limit) : interleaved
  return {

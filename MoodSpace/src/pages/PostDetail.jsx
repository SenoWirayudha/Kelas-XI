  ...
  useEffect(() => {
    ...
    getPost(id)
      .then((payload) => {
        ...
        const autoGenTags = payload.post.metadata?.autoGeneratedTags || []
        const matchedEntity = payload.post.metadata?.matchedEntity
        const useMatchedEntity = matchedEntity && matchedEntity.score >= 0.90 && matchedEntity.status === 'suggested'
        let externalQuery
        if (useMatchedEntity) {
          externalQuery = matchedEntity.title
        } else {
          const titleTokens = (payload.post.title || '').split(/\s+/).filter(Boolean)
          externalQuery = titleTokens.slice(0, 5).join(' ') || autoGenTags.slice(0, 3).join(' ') || 'design inspiration'
        }
        recommendedQueryRef.current = externalQuery
        const tags = [...(payload.post.tags || []), ...autoGenTags]
        const uniqueTokens = new Set()
        payload.post.title?.toLowerCase().split(/\s+/).forEach(t => uniqueTokens.add(t))
